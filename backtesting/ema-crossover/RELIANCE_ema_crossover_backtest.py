...
try:
    ...
    # Fallback to dummy data if API fails to show structure
    df = pd.DataFrame()

if df.empty:
    print("WARNING: No data returned from API. Generating synthetic data for structure demonstration.")
    dates = pd.date_range(start=start_date, end=end_date, freq='D')
    df = pd.DataFrame(index=dates)
    df['close'] = 2500 + np.cumsum(np.random.normal(0.5, 20, size=len(dates)))
...
if df_bench.empty:
    bench_close = close * 0.8 # Synthetic benchmark
else:
