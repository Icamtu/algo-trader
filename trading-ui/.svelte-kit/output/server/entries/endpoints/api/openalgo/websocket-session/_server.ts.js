    ...
    if (!configResult.response.ok || configResult.data.status !== "success" || !configResult.data.websocket_url) {
      return withNoStore(
        json(
          {
            status: "error",
            message: configResult.data.message || "OpenAlgo did not return a websocket configuration for this session."
          },
    ...
    if (!apiKeyResult.response.ok || apiKeyResult.data.status !== "success" || !apiKeyResult.data.api_key) {
      return withNoStore(
        json(
          {
            status: "error",
            message: apiKeyResult.data.message || "OpenAlgo did not return an API key for websocket authentication."
          },
          { status: apiKeyResult.response.status || 502 }
        )
      );
    }
    const websocketUrl = private_env.PUBLIC_WS_URL || configResult.data.websocket_url || DEFAULT_WS_URL;
    return withNoStore(
      json({
        status: "success",
        apiBaseUrl,
...
async function fetchOpenAlgoJson(url, cookie) {
...
function resolveApiBaseUrl() {
  return (private_env.INTERNAL_API_URL || private_env.PUBLIC_API_URL || DEFAULT_API_URL).replace(/\/+$/, "");
}
