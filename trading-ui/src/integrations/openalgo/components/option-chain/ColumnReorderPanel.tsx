...
import type { ColumnKey } from '@/integrations/openalgo/types/option-chain'
...
export function ColumnReorderPanel({
  ...
  return (
    <Dialog open={open} onOpenChange={(val) => {
      setOpen(val)
      if (val) setLocalOrder(columnOrder)
    }}>
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Drag columns to reorder. Movement is restricted within CE or PE groups.
          </DialogDescription>
        </DialogHeader>

        <div className="grid grid-cols-2 gap-4">
          <div>
            <h4 className="text-xs font-bold mb-2 text-green-500 uppercase tracking-wider">CALLS (CE)</h4>
          ...
          <div>
            <h4 className="text-xs font-bold mb-2 text-red-500 uppercase tracking-wider">PUTS (PE)</h4>
            <ScrollArea className="h-64 pr-2">
              <div className="space-y-1.5">
                {peColumns.map(key => (
                  <DraggableColumn
