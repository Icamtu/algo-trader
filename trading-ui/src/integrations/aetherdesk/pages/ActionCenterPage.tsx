...
export const ActionCenterPage = () => {
  ...
  const fetchData = useCallback(async (tabTarget = activeTab) => {
    try {
      ...
      // The backend returns { status: 'success', data: { orders: [], statistics: {} } }
      const ordersList = orderRes?.data?.orders || (Array.isArray(orderRes) ? orderRes : []);
    ...
    <div className="h-full flex flex-col bg-black text-white font-mono overflow-hidden relative">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between p-8 gap-10 relative z-20 shrink-0">
        ...
        <div className="flex items-center gap-16 border-l border-white/5 pl-16 hidden lg:flex">
          <IndustrialValue label="Pending_Signals" value={Array.isArray(orders) ? orders.filter(o => o.status === 'PENDING').length : 0} trend={+12} />
          <IndustrialValue label="Kernel_Auths" value={Array.isArray(orders) ? orders.filter(o => o.status === 'APPROVED').length : 0} />
              ...
              >
                  <VirtualizedDataTable
                    ...
                    renderExpandedRow={(order) => {
                      const safeRawData = (() => {
                        try {
                          return typeof order.raw_order_data === 'string'
                            ? JSON.parse(order.raw_order_data)
                            : (order.raw_order_data || {});
                        } catch (e) {
                          return {};
                        }
                      })();
                      const strategyFile = safeRawData.filename || 'N/A';
                      const strategyCode = safeRawData.code || '';
                          ...
                          <div className="grid grid-cols-1 gap-12 relative z-10">
                              ...
                              <>
                                 ...
                                 <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-10">
                                     {Object.entries(safeRawData).map(([key, value]) => {
                                        if (key === 'code' || key === 'filename') return null;
                                        return (
