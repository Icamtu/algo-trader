...
export const TradesPage: React.FC = () => {
  ...
  const fetchTrades = useCallback(async (showRefresh = false) => {
    ...
    try {
      ...
      if (tradesResponse && tradesResponse.status === 'success') {
        const rawTrades = Array.isArray(tradesResponse.data?.trades)
          ? tradesResponse.data.trades
          : (Array.isArray(tradesResponse.data) ? tradesResponse.data : []);

        setTrades(rawTrades);
        setTradesCount(rawTrades.length);
        if (selectedTrade) {
          const updated = rawTrades.find(t => t.orderid === selectedTrade.orderid);
  ...
  const totalValue = useMemo(() => {
    ...
    return trades.reduce((acc, t) => acc + (t.trade_value || 0), 0);
  }, [trades]);
  ...
  const filteredTrades = useMemo(() => {
    let result = Array.isArray(trades) ? trades : [];
    if (searchQuery) {
