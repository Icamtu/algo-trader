...
export class AetherAdapter {
  private async requestWithFallback<T>(
    primary: () => Promise<{ data: T }>,
  ...
  async getWebSocketApiKey() {
    const response = await webClient.post('api/websocket/apikey');
    return response.data;
  }
  ...
  async getSandboxPnLData() {
    const response = await webClient.get('api/v1/sandbox/pnl/api/data');
    return response.data;
  }
  ...
  async getAnalyzerData(startDate?: string, endDate?: string) {
    ...
    const response = await apiClient.get(`analyzer/data?${params.toString()}`);
    return response.data;
  }
  ...
  async approveActionCenterOrder(id: string | number) {
    const response = await webClient.post(`api/v1/actioncenter/approve/${id}`, {});
    return response.data;
  }

  async rejectActionCenterOrder(id: string | number, reason: string = 'Rejected by user') {
    const response = await webClient.post(`api/v1/actioncenter/reject/${id}`, { reason });
    return response.data;
  }

  async deleteActionCenterOrder(id: string | number) {
    const response = await webClient.delete(`api/v1/actioncenter/delete/${id}`);
    return response.data;
  }
  ...
  async approveAllActionCenterOrders() {
    const response = await webClient.post('api/v1/actioncenter/approve', { batch: 'all' });
    return response.data;
  }
  ...
  async getVolSurfaceData(params: { underlying: string; exchange: string; expiry_dates: string[]; strike_count: number }) {
    return this.requestWithFallback(
      () => webClient.post('vol-surface/api/surface-data', params),
      () => webClient.get('vol-surface/api/surface-data', { params })
    );
  }

  async getIVSmileData(params: { underlying: string; exchange: string; expiry_dates: string[] }) {
    return this.requestWithFallback(
      () => webClient.post('iv-smile/smile-data', params),
      () => webClient.post('iv-smile/api/smile-data', params)
    );
  }

  async getIVChartData(params: { underlying: string; exchange: string; expiry_date: string; days: number }) {
    return this.requestWithFallback(
      () => webClient.post('iv-chart/api/chart-data', params),
      () => webClient.get('iv-chart/api/chart-data', { params })
    );
  }
  ...
  async getMaxPainData(params: { underlying: string; exchange: string; expiry_date: string }) {
    return this.requestWithFallback(
      () => webClient.post('max-pain/api/pain-data', params),
      () => webClient.get('maxpain/api/data', { params })
    );
  }

  async getStraddleChartData(params: { underlying: string; exchange: string; expiry_date: string; days: number }) {
    return this.requestWithFallback(
      () => webClient.post('straddle-chart/api/data', params),
      () => webClient.get('straddle-chart/api/data', { params })
    );
  }
...
export const aetherAdapter = new AetherAdapter();
