...
export function VirtualizedDataTable<T>({
  ...
  const Row = ({ index, style }: any) => {
    ...
    const itemKey = keyExtractor ? keyExtractor(item, index) : (item as any).id || (item as any).timestamp || index;
  ...
  const InnerList = ({ height, width }: { height: number | undefined, width: number | undefined }) => (
    <List
      listRef={listRef}
  ...
  return (
    <div className={cn("flex flex-col h-full w-full overflow-hidden font-mono", containerClassName)}>
      ...
      <div className="flex-1 min-h-0 relative">
        <AutoSizer ChildComponent={InnerList} />
      </div>
    </div>
  );
}
