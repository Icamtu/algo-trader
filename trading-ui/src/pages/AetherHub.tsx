...
import { useSystemHealth, useOrders, usePositions } from "../features/aetherdesk/hooks/useTrading";
...
export default function AetherHub() {
  ...
  const coreModules = [
    { to: "/aetherdesk/orders", icon: ClipboardList, label: "Order_Registry", desc: "Trade execution & status map.", color: primaryColor, bg: primaryBg },
    { to: "/aetherdesk/strategy-registry", icon: Cpu, label: "Strategy_Registry", desc: "Central_Command for all trading cores.", color: secondaryColor, bg: secondaryBg },
    { to: "/aetherdesk/positions", icon: LayoutGrid, label: "Position_Matrix", desc: "Live exposure & MTM vectors.", color: primaryColor, bg: primaryBg },
    { to: "/aetherdesk/trades", icon: Zap, label: "Execution_Log", desc: "Verified operation history.", color: secondaryColor, bg: secondaryBg },
    { to: "/aetherdesk/holdings", icon: ShieldCheck, label: "Asset_Vault", desc: "Long-term inventory audit.", color: primaryColor, bg: primaryBg },
  ];

  const analysisModules = [
    { to: "/aetherdesk/option-chain", icon: Layers, label: "Option_Chain", desc: "Multi-strike derivatives grid.", color: primaryColor, bg: primaryBg },
    { to: "/intelligence/gex", icon: BarChart3, label: "GEX_Dashboard", desc: "Gamma exposure visualization.", color: secondaryColor, bg: secondaryBg },
    { to: "/aetherdesk/oi-profile", icon: LayoutGrid, label: "OI_Profile", desc: "Open Interest evolution map.", color: primaryColor, bg: primaryBg },
    { to: "/aetherdesk/oi-tracker", icon: Target, label: "OI_Tracker", desc: "Comparative OI distribution.", color: secondaryColor, bg: secondaryBg },
    { to: "/aetherdesk/vol-surface", icon: Activity, label: "Vol_Surface", desc: "Implied volatility surface map.", color: primaryColor, bg: primaryBg },
    { to: "/aetherdesk/iv-smile", icon: Activity, label: "IV_Smile_Lab", desc: "Cross-strike volatility skew.", color: secondaryColor, bg: secondaryBg },
    { to: "/aetherdesk/iv-chart", icon: BarChart3, label: "IV_History", desc: "Time-series Greek evolution.", color: primaryColor, bg: primaryBg },
  ];

  const strategyLabs = [
    { to: "/aetherdesk/max-pain", icon: Target, label: "Max_Pain_Lab", desc: "Strike-wise pain index audit.", color: secondaryColor, bg: secondaryBg },
    { to: "/aetherdesk/straddle-lab", icon: Layers, label: "Straddle_Lab", desc: "Synthetic future & premium tracking.", color: primaryColor, bg: primaryBg },
    { to: "/intelligence/historify", icon: Database, label: "Historify", desc: "Distributed data persistence.", color: secondaryColor, bg: secondaryBg },
  ];

  const systemModules = [
    { to: "/aetherdesk/action-center", icon: PlayCircle, label: "Action_Center", desc: "Semi-auto order approval gate.", color: isAD ? "text-primary" : "text-teal-500" },
    { to: "/aetherdesk/connectivity", icon: Key, label: "Access_Kernel", desc: "API key & Protocol manage.", color: primaryColor },
    { to: "/aetherdesk/broker", icon: Network, label: "Broker_Bridge", desc: "Secure gateway handshake.", color: secondaryColor },
    { to: "/aetherdesk/master-contract", icon: Database, label: "Symbol_Base", desc: "Broker contract database sync.", color: primaryColor },
    { to: "/aetherdesk/simulation", icon: Box, label: "Simulation", desc: "Isolated environment hypervisor.", color: secondaryColor },
    { to: "/aetherdesk/analyzer", icon: Activity, label: "Protocol_Analyzer", desc: "API payload & protocol audit.", color: primaryColor },
    { to: "/aetherdesk/logs", icon: History, label: "Telemetry", desc: "Kernel-level audit stream.", color: secondaryColor },
    { to: "/aetherdesk/health", icon: ShieldCheck, label: "Sanity_Audit", desc: "Host diagnostics & health.", color: primaryColor },
    { to: "/aetherdesk/playground", icon: TerminalIcon, label: "Playground", desc: "API terminal & protocol test.", color: secondaryColor },
  ];

  const ordersData = orders?.data || orders;
  const positionsData = positions?.data?.positions || positions?.positions || (Array.isArray(positions?.data) ? positions.data : (Array.isArray(positions) ? positions : []));

  const stats = [
    { label: "Module_Status", value: (health?.status === "success" || health?.data?.status === "success") ? "HEALTHY" : "SYNCING", icon: Cpu, color: secondaryColor },
    { label: "Active_Orders", value: (Array.isArray(ordersData?.trades) ? ordersData.trades.length : 0), icon: Activity, color: primaryColor },
    { label: "Live_Positions", value: (Array.isArray(positionsData) ? positionsData.length : 0), icon: Layers, color: secondaryColor },
    { label: "Security_Auth", value: "VERIFIED", icon: Fingerprint, color: isAD ? "text-primary" : "text-teal-500" },
  ];

  return (
    <div className="p-6 space-y-8 animate-in fade-in duration-700 pb-24 relative">
      {/* Header & Stats Bundle */}
      <div className="flex flex-col xl:flex-row gap-12 items-start justify-between">
        <div className="space-y-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className={cn("text-4xl font-black font-mono uppercase tracking-[0.4em]", isAD ? "text-primary" : "text-teal-500")}>AetherDesk_Mission_Control</h1>
              <p className="text-[10px] font-mono text-muted-foreground uppercase tracking-[0.3em] mt-2 opacity-60">Unified_Operational_Agent_Array // Aether_V2_Stable</p>
            </div>
          </div>
           ...
           <AetherPanel className="mt-8 border-rose-500/20 bg-rose-500/5">
              <div className="flex items-start gap-4">
                 ...
                 <div className="space-y-2">
                    ...
                    <Link to="/aetherdesk/master-contract" className="inline-block text-[8px] font-mono font-black text-rose-500 hover:text-white underline uppercase tracking-widest mt-2">
                       RESOLVE_NOW →
                    </Link>
                 </div>
              </div>
